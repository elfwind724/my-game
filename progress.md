...
## 最新进展（2026-02-24 自动化建造经济）
- [x] 自动建造升级为“岗位驱动”：
  - `AutoBuildSettings` 新增 `queueCap / autoAssignBuilders / desiredBuilderCount / crewMode`。
  - 自动建造并发现在受“施工班组可用人数”约束（可配置为`仅工坊`或`工坊+空闲`）。
- [x] 自动派工闭环：
  - 自动建造开启后可按目标人数自动把空闲驻守伙伴转为工坊岗位；
  - 无施工任务时可回落多余工坊岗位（避免长期占用）。
- [x] 建造时间与施工状态增强：
  - 自动施工夜晚暂停时，进行中的自动任务会显示“夜间暂停”并冻结进度；
  - 施工速度新增“是否有施工伙伴”系数，体现委派价值。
- [x] 基地管理面板新增委派控制：
  - 新增队列上限调节；
  - 新增自动派工开关；
  - 新增施工来源模式切换（工坊专职 / 工坊+空闲）；
  - 新增目标施工人数调节与工坊岗位/目标显示。
- [x] 编译与自动化测试：
  - `npm run -s build` 通过；
  - `npm run -s test:game` 通过，截图文件：`/Users/fengnian/my-game/output/web-game/shot-0.png`；
  - `errors-0.json` 为空，未出现新控制台报错。

### 下一步建议
- [ ] 增加“自动建造预案”页（防线优先/经济优先/人口优先）一键切换规则与岗位目标。
- [ ] 在施工点上显示“施工者姓名+剩余秒数”，进一步提升生活感。
- [ ] 给自动派工增加“最少保留空闲人数”阈值，避免白天活动与施工抢人。
  - 任务满额时显示提示并阻止继续接取。
...
## 最新进展（2026-02-24）
- [x] 建造生态链核心重构（对标“前置依赖 + 分区放置 + 维护输入 + 产能联动”）：
  - 新增 `src/data/buildingEcology.ts`，为基地内建筑定义角色、链路、解锁日、前置条件、内/外圈分区、日常维护与输入。
  - `BaseSystem` 新增 `getBuildChainStatus` / `validateBuildPlacement`，建造合法性不再只看资源。
- [x] 基地状态接入生态指标：
  - `GameState.base` 增加生态完整度、维护率、产能率、五维评分与警告列表。
  - `BaseSystem.refreshBaseState()` 写回生态指标，`applyDailyTick()` 按链路和资源短缺动态缩放产出并先扣维护/输入成本。
- [x] 建造交互改造：
  - `GameScene` 建造预览与放置接入链路校验，阻塞时给出明确原因（天数未解锁/缺前置/分区错误/电力不足/资源不足）。
  - 建造模式提示面板增加“链路+分区+阻塞原因”。
- [x] 建造面板信息改造：
  - 建筑卡片增加链路信息和分区标签。
  - 不满足前置时按钮显示“链路未解锁”，避免误导。
- [x] 基地管理面板增加生态摘要：
  - 显示生态链完整度、维护率、产能率、生态评分与首条风险警告。
- [x] 构建与回归：
  - `npm run build` 通过。
  - `npm run test:game` 通过，截图与状态输出正常，控制台错误为 0。

### 下一步建议
- [ ] 将“结构完整性”做成网格连通校验（围墙闭环、关键设施需在护墙内）以进一步接近《七日杀》基地逻辑。
- [ ] 做“建造升级树”：每类建筑改为 Tier 升级（同坐标升级）并接入工坊/科技前置。
- [ ] 为生态链断裂添加即时告警图标（HUD）与地图热区高亮（缺维护、缺输入、缺供电）。

## 最新进展（2026-02-24 · 闭环防线）
- [x] 新增“防线闭环/破口”系统（BaseSystem）：
  - 通过 64 网格连通判定“外圈是否可直达基地核心”（破口判定）。
  - 增加环形采样覆盖率（防线覆盖不足会触发惩罚）。
  - 产出 `structureIntegrity / structureCoverage / structureBreachOpen` 指标。
- [x] 防线指标接入基地生态：
  - 生态评分中的防御分受防线完整度乘区影响。
  - 生态警告新增“防线存在破口 / 覆盖不足”提示。
- [x] 夜战惩罚落地：
  - 破口开启时，炮塔射速变慢、伤害降低，并周期提示“防线破口，炮塔效率下降”。
- [x] UI 可视反馈：
  - 左上资源串新增 `防XX%` 与 `🧱破口` 标记。
  - 基地管理面板新增“防线闭环% / 完整度% / 破口状态”。
- [x] 构建与冒烟：
  - `npm run build` 通过。
  - `npm run test:game` 通过，截图生成，控制台错误为 0。

### 下一步（已准备）
- [ ] 闭环升级到“多层防区”（外墙/内墙）并把敌人路径选择接入“优先攻击薄弱段”。
- [ ] 增加“墙体段耐久热力图”与“一键维修”交互。
- [ ] 把 gate 做成可开关状态（白天开、夜晚自动关），并影响连通判定。

## 最新进展（2026-02-24 · 同座标升级树）
- [x] 建筑升级树数据化：新增 `BUILDING_MORPH_UPGRADES`（分支改装）与 `getBuildingTierTechRequirements`（升阶科技前置）。
- [x] 升级改为“同座标操作”：
  - 建造模式点击已占用格时，优先进入升级判定（而非直接报冲突）。
  - 支持两类升级：同ID升阶（T+1）与分支改装（如炮塔->激光炮塔）。
- [x] 工坊/科技前置已接入：
  - T2 起至少需工作台；T3 需工作台+雷达；T4 需更高科技前置。
  - 分支改装按路线要求工作台/雷达/弹药工厂/天数。
- [x] 升级成本与反馈：
  - 升级使用独立耗材计算（非整价重建），并在建造预览显示“同格行为/升级耗材”。
  - 升级成功后原地替换贴图/属性/记录，炮塔保留自动等级并更新基础参数。
- [x] UI 提示增强：
  - 建造卡与建造提示新增“升级树”文案，明确可升路线。
- [x] 验证通过：
  - `npm run build` 通过。
  - `npm run test:game` 通过，console error = 0。

### 下一步建议
- [ ] 为每个建筑加“右键/长按打开升级面板”（显示当前等级、下一节点、前置、耗材、收益对比）。
- [ ] 升级路线加入“拆解回退”与“分支重置成本”，形成可逆策略。
- [ ] 给升级成功加专属VFX/音效与短暂数据浮层（DPS/耐久变化）。

## 最新进展（2026-02-25 · 物流/电网诊断可视化）
- [x] **节点级基地诊断数据落地**：
  - `BaseState` 新增 `nodeDiagnostics` 及计数字段（缺维护/缺输入/缺电）。
  - `BaseSystem.refreshBaseState()` 现会按建筑逐节点分配资源与电力，产出具体故障节点。
- [x] **同一份诊断驱动 HUD + 地图**：
  - 左上资源诊断摘要加入 `维/输/电` 计数。
  - 小地图新增故障节点标记：
    - 缺维护：粉红方块
    ...
    - 多故障：紫粉环形高亮
- [x] **调试快照扩展**：
  - `render_game_to_text` 新增 `baseDiagnostics` 字段，便于自动化回归校验。

### 本轮验证
- [x] `npm run -s build` 通过。
- [x] `npm run -s test:game` 通过（`output/web-game/errors-0.json` 为空数组）。
- [x] 回归截图：`output/web-game/shot-0.png`（HUD 与小地图诊断信息已渲染）。

### 下一步建议
- [ ] 给故障节点加“点击定位/摄像机跳转”，支持一键巡检。
- [ ] 增加“诊断筛选层”（仅看缺电 / 仅看缺输入），减少后期密集建筑时视觉噪音。
- [ ] 在基地管理面板新增“故障节点列表（按严重度排序）”，联动一键派工维修。

## 最新进展（2026-02-25 · 故障节点交互：跳转定位 + 一键维修/派工）
- [x] **小地图故障节点可点击**：
  - 点击后会命中最近故障点（阈值12px）并打开“节点诊断操作面板”。
- [x] **镜头跳转定位**：
  - `GameScene.focusCameraOnWorldPoint(x, y)` 已实现，点击节点会短暂聚焦并播放定位脉冲，再自动回到跟随玩家。
- [x] **一键维修（应急补给）**：
  - `BaseSystem.quickRepairDiagnosticNodeByCoord(x, y)` 已实现。
  - 按节点短缺资源计算应急采购成本（消耗 bitcoin），补足该节点短缺资源后刷新生态状态。
- [x] **一键派工**：
  - `BaseSystem.dispatchCrewToDiagnosticNodeByCoord(x, y)` 已实现。
  - 按故障类型自动匹配岗位优先级（供电/工坊/农务/医疗），优先派空闲驻守伙伴。
- [x] **节点短缺量数据化**：
  - `BaseNodeDiagnostic` 新增 `shortageAmounts`，用于维修成本与补给精确计算。

### 本轮验证
- [x] `npm run -s build` 通过。
- [x] `npm run -s test:game` 通过，`output/web-game/errors-0.json` 为空数组。

## 最新进展（2026-02-24 · 伙伴自动职责 + 面板字体放大）
- [x] 基地面板可读性提升：
  - 基地管理面板文字整体放大（尤其自动建造与生态/防线摘要区）。
  - 自动分配后新增结果提示气泡，直接反馈分配到的职责人数。
- [x] 新增伙伴自动职责层（不替代原岗位）：
  - `CompanionData` 增加 `autoDuty`（建筑工 / 拾荒者 / 防御者 / 后勤）。
  - `BaseSystem` 增加职责识别与标签接口，按职业关键词自动判定职责。
- [x] 一键自动分配扩展：
  - `BaseSystem.autoAssignBaseCompanions()` 现在会同时分配岗位 + 自动职责。
  - 分配结果消息包含三类关键职责人数（建筑工/拾荒者/防御者）。
- [x] 建筑工/防御者接入自动施工：
  - 自动施工派工优先建筑工，其次防御者；
  - 建筑工和防御者有施工速度加成；
  - 防御者会提高墙体/炮塔类规则优先级与保底目标，自动防线建设更积极。
- [x] 拾荒者接入自动拾取怪物掉落：
  - `LootSystem` 新增外部收集点机制；
  - 基地中的拾荒者会成为掉落吸附点，自动把周边掉落吸入并结算资源。

### 本轮验证
- [x] `npm run -s build` 通过（含 `tsc`）。
- [x] `npm run -s test:game` 通过，`output/web-game/errors-0.json` 为空数组。
- [x] 手动回归截图：
  - `/Users/fengnian/my-game/output/web-game/shot-base-panel-after-start.png`（可见基地面板打开）。

## 最新进展（2026-02-24 · 自动施工可视化与委派闭环补强）
- [x] **自动委派可回收**：
  - 当“目标施工人数=0”且当前无在建任务时，自动派工会把工坊施工位回退为空闲，不再长期占岗位。
- [x] **施工时间可视化增强**：
  - 工地进度牌新增“剩余秒数”显示（排队显示“等待派工”），让建造时间感知更直观。
- [x] **施工现场动态反馈**：
  - 施工中的工地会周期出现脉冲特效，区分自动/手动施工颜色。
  - 被指派施工的伙伴会显示施工标记，形成“有人在干活”的基地生活感。
- [x] **基地管理面板补充施工队列预览**：
  - 自动建造区新增施工队列小窗，显示当前前几条任务的“建筑名 / 状态 / 剩余秒数”。
...
## 最新进展（2026-02-24）
- [x] 基地管理面板字体与可读性再次提升：`BasePanel` 提高字号基线、放宽字体缩放上限、加宽桌面面板宽度，并重排摘要区行距。
- [x] 自动分工信息可视强化：新增独立“自动分工”信息块，显示职责计数与职责行为提示，补充“自动派职后空闲伙伴自动参与基地作业”的说明行。
- [x] 自动派职联动自动建造：在基地面板开启“自动派职”时，同时开启 `autoBuild.enabled` 与 `autoAssignBuilders`，减少手动开关步骤。
- [x] 基地面板打开时做一次即时派职同步：`autoAssignDuties` 开启状态下，打开面板会先执行一次 `autoAssignBaseCompanions()`，避免显示与实际岗位不同步。
- [x] 职业识别关键词扩展（繁简兼容）：`建筑工/建築工/电工/電工/工程师/工程師`、`拾荒/猎人/獵人/快递/快遞/回收`、`防御/防禦/守卫/守衛`，提高自动分工命中率。
- [x] 本地验证：`npm run build` 通过；已生成截图 `output/basepanel-after-v2-ingame.png` 用于核对字体与自动分工展示。

### 下一步建议
- [ ] 在“伙伴名单”有 6+ 人时再补一轮真机截图，确认卡片字体与滚动条在高分屏下仍清晰。
- [ ] 给“自动分工”增加手动权重滑杆（建筑工/拾荒者/防御者比例），让自动派职更可控。

## 最新进展（2026-02-25 · 金来源增强 + 自动派职修正）
- [x] 白天“金(金属)”来源增强并可视化：
  - 河流点位新增**淘金**收益：伙伴河流活动与河流探索结算可直接产出金属。
  - 山洞点位强化为**挖矿**收益：洞穴探索收益中金属权重提升，提示文案改为“山洞挖矿”。
  - 伙伴白天浮字与生活提示更新：`河流淘金 / 山洞挖矿 / 挖矿发现`，便于玩家直接理解金来源。
- [x] 伙伴协助任务收益扩展：
  - 白天协助任务中，`fishing/adventure` 现在可触发金属资源协助（淘金协助/矿脉协助）。
- [x] 自动派职目标算法修正（小队人数下也能分配建筑工）：
  - `BaseSystem.computeAutoDutyTargets` 改为当自动建造开启且存在施工目标/规则时，建筑工最低保底可达 1（2人小队也可出现建筑工）。
  - 拾荒者保底从“2人即保底1”调整为“3人以上保底1”，避免 2 人小队被“防御+拾荒”锁死，无法出现建筑工。
- [x] 伙伴行为与职责联动增强：
  - 新增 `getResidentBehaviorForCompanion`，以 `autoDuty`（建筑工/拾荒者/防御者/后勤）影响驻守行为分布，不再仅看岗位；即使岗位暂时是空闲，也能表现出职责倾向。
- [x] 基地面板可读性继续提升：
  - 自动分工摘要与生态摘要字号再放大一档。
  - 新增“金来源”提示行：`河流淘金 / 山洞挖矿 / 城区搜刮 / 夜战掉落`。

### 本轮验证
- [x] `npm run build` 通过。
- [x] 运行 Playwright 自动冒烟（`web_game_playwright_client.js`）成功生成状态与截图文件；本次自动截图出现黑屏（已记录），但 `state-0/1.json` 正常返回菜单态且 `errors-0.json` 为空。

### 下一步建议
- [ ] 针对“自动派职”再加一个面板内筛选：仅显示建筑工/拾荒者/防御者，便于玩家直接确认分配结果。
- [ ] 在河流/山洞点位标牌上增加“可产出金属”小图标，进一步降低理解成本。

## 最新进展（2026-02-25）
- [x] **自定义主角四方向接入（可回退）**：新增 `src/data/customHero.ts`，统一维护 `主角正面/向左/向右/背面` 资源路径、运行时贴图键与可用性检测。
- [x] **BootScene 运行时处理大图主角**：加载原始四张主角图后自动做边缘背景剔除、Alpha包围盒裁切、缩放落位到 32x32，并生成 `player_custom_[s/w/e/n]`。
- [x] **移动方向贴图切换**：`PlayerSystem` 增加 4 向方向判定并切换 `player_custom_*`，保留 V2 动画与旧主角路径作为 fallback，避免资源缺失时崩溃。
- [x] **肖像联动**：菜单与装备面板头像优先使用 `player_custom_s`。
- [x] **构建与自动化回归**：`npm run build` 通过；Playwright 冒烟脚本无新报错（当前测试脚本仍会卡在“每日挑战”弹窗流程，后续需补专用测试入口）。

### 下一步建议
- [ ] 给自定义主角做“真实透明底”导出版本（当前是运行时扣底，边缘质量受原图背景影响）。
- [ ] 增加自定义主角 8 向/行走帧（目前为 4 向静态切图）。
- [ ] 为测试脚本补一个 `__debug_select_day_challenge` 钩子，稳定进入实战场景做角色显示截图回归。

## 最新进展（2026-02-25 / 回合2）
- [x] **主角左移朝向修复**：`PlayerSystem` 朝向判定改为“横向优先 + 键盘方向优先”，解决“行走中偶发不是左侧形象，停下才恢复左侧”的问题。
- [x] **主角朝向调试可观测性**：`render_game_to_text` 新增 `player.texture` 和 `player.customFacingDir` 字段，便于自动化回归直接验证四方向贴图切换。
- [x] **子弹素材策略收口**：`BootScene` 不再从 Kenney 表里覆盖 bullet 贴图；子弹贴图统一强制走程序化像素魔法风，避免素材包误映射导致观感混乱。
- [x] **魔法化弹幕贴图重绘**：重绘 `bullet / scatter / pulse / flame / pierce / cannon / frost / chain` 八种像素贴图，统一魔法视觉语言。
- [x] **战斗魔法演出增强**：弹道映射补全（含 flamethrower/laser/chain 识别），命中与枪口新增光环/符文扩散层，强化技能释放的“法术感”。
- [x] **回归验证**：`npm run build` 通过；自动化状态快照显示左移后 `player.texture = player_custom_w`。

### 下一步建议
- [ ] 为 4 向主角补“行走逐帧动画”（当前是方向切图，非步行动画）。
- [ ] 补一个“战斗演示”测试钩子（直接刷一组假敌人）用于子弹/VFX自动截图回归。
- [ ] 按技能树进一步做专属法术弹道（比如链电分叉、冰锥扇面、奥术轨道弹）。

## 最新进展（2026-02-25 夜间续）
- [x] 掉落文本防拥挤二次优化（`LootSystem`）：新增“同屏标签上限 + 最近优先”策略。
  - 移动端（宽<=540）：最多显示 2 个资源标签
  - 中屏（<=960）：最多显示 3 个
  - 大屏：最多显示 5 个
  - 其余标签做平滑淡出并自动隐藏，避免满屏 `x1` 文本叠在一起。
- [x] 自定义主角朝向修复（`PlayerSystem`）：方向判定改为基于 `body.velocity` 实际速度，修正“向左行走时偶发不保持左朝向”的问题。
- [x] 编译回归：`npm run build` 通过。
- [x] 实机回归：
  - 使用内置 Playwright 进入 Day1 干净存档，触发 `__debug_spawn_loot_preview` 校验掉落标签行为；
  - 控制台错误检查为 0；
  - 说明：`develop-web-game` 的外部脚本在本机仍会产出黑屏截图（WebGL 捕获兼容问题），本轮已改用内置 Playwright 截图验证。

## 最新进展（2026-02-26 基地面板稳定性修复）
- [x] 基地管理面板层级提升：`SlidePanel` 深度从 `2800` 调整为 `6200`，避免与白天挑战/事件层叠导致交互错位。
- [x] 基地管理字号上限收敛：`BasePanel.getFontBoost()` 改为更保守的分端上限（桌面/横屏/竖屏分开），避免文字过大引发重叠。
- [x] 顶部信息区重构：自动建造信息改为“紧凑摘要 + 可展开详情”，并增加顶部区高度硬限制，确保伙伴列表区域始终保底可见。
- [x] 修复“运营详情按钮无效”：移除 `show()` 中对 `showAutoBuildDetails` 的强制重置，展开/收起状态可正确切换。
- [x] 保持伙伴系统可用：验证 `伙伴名单` 渲染、滚动区域、`▸ 详情` 资料卡都可正常打开。

### 本轮验证
- [x] `npm run build` 通过（TypeScript + Vite）。
- [x] Playwright 本地回放验证：
  - 开局 -> 注入测试伙伴（`__debug_seed_base_companions(6/8)`）-> 打开基地面板；
  - 桌面与竖屏下均能看到伙伴名单，且列表区不被顶部信息吞掉；
  - 详情卡可正常弹出，显示伙伴完整属性与档案字段。

## 最新进展（2026-02-27 早）
- [x] 地图背景分类拼接重构：`drawClassifiedWorldBaseMap` 从“高频碎片裁切”改为“分区大块拼贴（城市/森林/雪地）+ 全图保底铺满”，减少杂乱感并保持分类边界。
- [x] 背景强制重建生效：`world_base_map` 继续使用 `drawCanvasTextureForce`，避免旧覆盖图残留导致新分类背景不显示。
- [x] 可读性微调：降低道路/暗角压黑强度，保留河道与主路轮廓但不压死背景细节。
- [x] 运行时验证：通过浏览器执行脚本确认 `world_base_map` 为 `2000x1500` 且城市/森林/雪地采样点颜色已分离。
- [ ] Playwright PNG 截图在当前环境仍出现黑屏（WebGL 捕获链路问题），需以浏览器实机画面为准继续验收。

## 最新进展（2026-03-07）
- [x] 武器成长里程碑系统一期：新增 `src/data/weaponMilestones.ts`，为基础射线/散射/脉冲/烈焰/穿透/能量炮/环绕/圣水/闪电环/回旋镖等武器定义 `Lv3 / Lv5 / Lv8` 跃迁节点。
- [x] 战斗实装接线：`WeaponSystem` 现在会把里程碑奖励接入真实武器配置（伤害、射速、投射物、速度、射程、签名弹幕强度/频率），升级后不再只是隐性数值增长。
- [x] 升级面板提示增强：`EvolutionSystem` 为升级武器卡追加跃迁预览，跨关键等级时会在卡面底部显示 `Lv.X跃迁：...`，并把里程碑权重计入 DPS 预估。
- [x] 选卡战斗反馈增强：`GameScene` 在玩家跨过武器里程碑时显示 `武器跃迁` 飘字与简短说明，并闪屏提示，强化升级体感。
- [x] 验证：`npx tsc --noEmit` 通过，`npm run build` 通过；本地 Vite + Playwright 轻量冒烟确认游戏场景可正常进入且控制台无报错。
- [x] 伙伴成长里程碑系统一期：新增 `src/data/companionMilestones.ts`，为坦克/狙击/医师三类伙伴定义 `Lv5 / Lv10 / Lv20` 阶段跃迁，覆盖战斗火力、驻守收益、自动施工与拾荒半径。
- [x] 伙伴收益实装接线：`GameScene` 已把伙伴里程碑接入夜间驻守伤害、白天自动产出、居民协助奖励、自动施工速度与拾荒吸附半径，成长不再只体现在后台等级数字。
- [x] 伙伴面板提示增强：`Panels` 的伙伴列表卡片与详情卡新增“当前阶段 / 下阶段”信息，玩家能直接看到当前成长档位与下一次跃迁目标。
- [x] 武器里程碑视觉层二期：主武器与 VS 主动武器现在都会把里程碑阶段写入弹体元数据，按阶段提升弹体尺寸、轨迹摆动、拖尾密度、枪口闪光与命中爆闪强度。
- [x] 弹幕形态反馈增强：`GameScene` 的 `trail / muzzle / impact` VFX 已接入 `milestoneStage + vfxIntensity`，高阶段武器会出现更明显的符文尾迹、侧向光环、二次冲击圈与交叉爆闪，不再只是“同一种子弹变多了”。
- [x] 升级卡底部预览区稳定化：`LevelUpPanel` 现在为 `previewTextCN` 预留固定底部区域，并将预览文本限制在稳定两行内，避免与描述区和卡片底边重叠。
- [x] 武器签名弹幕按类型分化：`WeaponSystem.maybeFireSignaturePattern` 现已按里程碑阶段为 `pulse / flame / pierce / lightning_ring / chain` 做专属形态，分别强化为“脉冲格栅”“焚风火环”“十字裂束”“雷网扩散”和“链弧风暴”。
- [x] 里程碑不再只加数量：高阶段武器现在会追加侧向副线、正交切割、对角交叉、前后雷幕与多层环状补射，玩家能从屏幕几何结构直接分辨武器已进入哪一档。
- [x] UI 可读性优化一期：`BasePanel` 顶部摘要区字体下限整体上调，并把操作按钮统一为紧凑样式，减少信息密度导致的重叠。
- [x] 伙伴列表卡片重排：`BasePanel` 的伙伴卡片高度提升，名称/职业/签名/职责文案加入裁切，年龄徽标与状态按钮重新排位，避免长文本顶撞按钮区。
- [x] 建造面板卡片重排：`BuildPanel` 的建筑卡高度增加，标题/成本/说明文字最小字号上调，说明区改为更稳定的两行预算，底部标签与操作按钮移入独立 footer 区。
- [x] 验证：`npx tsc --noEmit` 通过，`npm run build` 通过；本轮改动未破坏现有发射与构建链路。
- [x] 每日挑战卡底部重排：`GameScene` 为三路分支卡新增独立奖励 footer，奖励预览改为紧凑摘要并与 `XP / 永久精通` 分层显示，修复桌面分辨率下底部文字互相覆盖。
- [x] 主角左移闪动修复：`AnimationSystem` 不再对自定义四向主角贴图执行通用 `setFlipX`，避免与 `PlayerSystem` 的换贴图逻辑互相打架，解决左移时左右来回闪动。
- [x] 基地管理热键双触发修复：移除 `GameScene` 中与 `UIScene` 重复的 `T / H / J` 面板热键监听，修复 `T` 面板一按就弹出又收回的问题。
- [x] 基地空 roster 布局修复：`BasePanel` 在无伙伴时也会渲染独立空态卡片，不再只剩一大片空白；同时给出招募提示和当前推荐施工方向。
- [x] 自动建造增强一期：新增 `BaseSystem.getConstructionAdvisorLines / applyRecommendedBuildPlan`，基地管理面板加入 `推荐施工` 按钮，可按人口、电力、防线、食水、仓储状态自动生成更合理的建造目标与施工优先级。
- [x] 炮台系统重构一期：`GameScene.updateTurrets` 不再让所有炮台发同一种脉冲弹，现已按炮台类型区分为脉冲连射、激光/狙击穿透、减速冰弹、追踪导弹、烈焰扇面五类火力几何。
- [x] 瞭望塔链路增幅：炮台会读取附近 `watchtower` 数量与自身阶段，动态提高伤害、射速、射程，并在高阶段触发更复杂的副弹道和压制覆盖。
- [x] 炮台命中效果实装：导弹炮会触发范围爆炸，减速炮会施加范围冰缓，烈焰炮会附加灼烧，激光/狙击炮具有穿透层数，不再只是命中即消失。
- [x] 建造推荐扩展到高阶火力链：`BaseSystem.applyRecommendedBuildPlan` 现会根据天数、供电和基础炮台数量自动补入瞭望塔、激光炮、烈焰炮、狙击巢和导弹炮，基地管理建议也会提示“补瞭望塔 / 补高阶火力”。
- [x] 建筑说明补全：`Panels` 已为 `sniper_fire / flame_fire / radar_boost` 增加用途与效果说明，建造面板能正确解释高阶炮台与瞭望塔的作用。
- [x] 掉装系统重构一期：`GearLootSystem` 已升级为暗黑式 affix 生成器，装备按敌源主题、武器类型、稀有度和物品等级生成前后缀，并对 `射程 / 穿透 / 链电 / 爆裂范围` 这类战斗关键维度提供真实词条。
- [x] 传说/神话专属词条：高稀有装备现在会抽取武器专属传奇能力，如 `镜返协议 / 交织矩阵 / 风暴之眼 / 彗星坠落`，不再只是同一套属性倍率换颜色。
- [x] 实战接线完成：装备词条现已接入 `WeaponSystem` 与 `GameScene.bulletHitEnemy`，真实影响武器射程、穿透次数、链电弹射数和爆炸半径，掉宝和战斗体感形成闭环。
- [x] 仓库展示增强：`GearVaultPanel` 已增加来源主题、词缀摘要与更完整的属性摘要，玩家可以直接从面板判断装备价值，而不是只看 `伤/速/弹+` 三个数字。
- [x] 掉落反馈增强：高品质装备掉落时，场景飘字会同时展示装备名称、来源主题和核心词缀摘要，掉宝感更接近刷装游戏。

### 下一步建议
- [ ] 继续把掉装系统拉向暗黑层级：加入 `套装 / 词缀池分层 / 世界首领专属掉落 / 词缀重铸`，让高端装备追求不只看稀有度。
- [ ] 把伙伴里程碑继续推到战斗视觉层：不同阶段触发更明显的驻守弹体形态、辅助特效和命中演出，让玩家肉眼可见“伙伴变强了”。
- [ ] 继续扩展剩余武器类型的阶段专属签名形态：`pistol / shotgun / rocket / orbit / holy_water / boomerang / frost` 仍可进一步从“同类散射”升级成更强识别度的 build 几何结构。
- [ ] 对 `BasePanel / BuildPanel / LevelUpPanel` 做一次实机截图回归，按桌面和竖屏各留 1 组对照图，继续清理剩余拥挤点。

## 最新进展（2026-03-07 弹幕视觉深化）
- [x] 弹体素材重画：`BootScene.generateProjectileSprites()` 已重绘 `bullet / scatter / pulse / flame / pierce / cannon / frost / chain / orbit / holy / boomerang` 11 类 fallback 弹体，使其在没有外部专属素材时依然具备更稳定的像素识别度。
- [x] 武器-弹体映射修正：`GameScene.getVSBulletTexture()` 现在优先按武器 ID 识别 `orbit / holy_water / boomerang`，不再被 `special` 覆盖误判成通用烈焰或穿透弹。
- [x] Archetype 视觉分型补全：`resolveBulletVfxArchetype()` 新增 `orbit / holy / boomerang` 三类弹幕语言，并接入真实 `trail / muzzle / impact` 表现。
- [x] 技能特征强化：不同武器 archetype 现已拥有更明显的视觉签名：
  - `holy`：圣环、十字闪、净化光爆
  - `orbit`：双侧轨道星屑、旋涡散开
  - `boomerang`：弧刃残影、回旋斩痕
  - `pulse`：脉冲格点与方波冲击
  - `scatter`：碎片喷散与短促枪口散裂
- [x] 这轮不是纯装饰：新增的视觉语言已经接在真实弹体发射链路上，玩家看到的几何差异就是实际战斗覆盖差异。
- [x] 验证：`npx tsc --noEmit` 通过；`npm run build` 通过。

## 最新进展（2026-03-07 敌人与 Boss 曲线强化）
- [x] 基础敌种行为差异强化：`runner` 现会折线突进，`swarm` 会近距离成群扑袭，`heavy` 会蓄力冲撞，`ranged` 会环绕走位射击，普通夜战不再只是同速追击。
- [x] 波次编成节奏重写：`WaveSystem` 现会按固定节拍插入特化威胁包，提高远程、重装、自爆、治疗、潜行、精英以及高阶敌种的出场密度，让每一波都更像“有结构的攻势”而不是随机堆怪。
