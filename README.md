...
`Netlify` 在中国大陆网络环境下经常不稳定或无法直连，建议改为以下方案之一：

1. `Cloudflare Pages`（通常比 Netlify 更稳，接入简单）
2. `Vercel`（部分地区可用性一般，需实测）
3. 国内云静态托管 + CDN（推荐长期方案）：
   - 腾讯云 `COS + CDN`
   - 阿里云 `OSS + CDN`
   - 华为云 `OBS + CDN`
...
1. `npm run build`
2. 将 `dist/` 上传到 `COS/OSS/OBS` 存储桶
3. 绑定 CDN 域名并开启缓存压缩（`br/gzip`）
4. 如果用自定义域名长期对大陆用户服务，按平台要求完成备案

### 首屏与移动端卡顿建议

1. 开启 CDN 缓存并配置长缓存：
   - `assets/*.png, *.webp, *.js, *.css` 缓存 30 天+
2. 减少首包体积：
   - 优先 WebP/压缩纹理
   - 清理未使用素材
3. 移动端默认开启低特效档（当前版本已自动按设备性能降载）
